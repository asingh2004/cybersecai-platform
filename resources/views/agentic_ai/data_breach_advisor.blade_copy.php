...
@php
    ...
    // Normalize regulations so Blade never receives a string.
    $selectedRegulationsRaw = $selectedRegulations ?? [];
...
<style>
  ...
  .determination-pill svg { width: 18px; height: 18px; }
  .copy-btn {
    border-radius: 999px;
        ...
        <div class="row m-0">
          ...
          <div class="col-md-9 mt-4 mt-sm-0 pl-4 pr-4">
            ...
            <div class="hero-card mb-4">
              ...
              <p>Rapidly assess suspected privacy or security incidents. Paste narrative or upload evidence (PDF / email), and let the Agentic AI map the right breach obligations based on your company profile.</p>
              ...
              <div class="col-12">
                <div class="input-card">
                  ...
                  <form method="POST" action="{{ route('agentic_ai.compliance.breach.run') }}" enctype="multipart/form-data" id="breachForm">
                    ...
                    <div class="mb-4">
                      <label class="form-label fw-semibold">Or attach supporting file (PDF, TXT, EML)</label>
                      <div class="dropzone" id="dropzone">
                        <input type="file" name="evidence_file" id="fileInput" accept=".pdf,.txt,.eml,.msg,.rtf,.html">
                        <div class="icon"><i class="fa fa-cloud-upload"></i></div>
                        <div><strong>Drag & drop</strong> or click to upload. Max 6 MB.</div>
                        <div class="text-muted mt-1" id="fileMetaText">We’ll auto-convert PDFs/emails to text securely.</div>
                      </div>
            ...
            @if($result)
              <div class="result-section" id="resultAnchor">
                ...
                <div class="steps-wrapper">
                  ...
                  <p class="text-muted mb-3">Click on a step to view detailed guidance generated for your jurisdiction.</p>
                  ...
                  <div class="row mt-3">
                    <div class="col-md-6">
                      <div class="summary-card h-100">
                        ...
                        <ul class="list-tight">
                          @foreach(($result['determination']['authority_notifications'] ?? []) as $notice)
                            <li><strong>{{ $notice['authority'] ?? '' }}</strong> — {{ $notice['deadline'] ?? '' }}<br><small class="text-muted">{{ $notice['rationale'] ?? '' }}</small></li>
                          @endforeach
                          @if(empty($result['determination']['authority_notifications']))
                    ...
                    <div class="col-md-6">
                      <div class="summary-card h-100">
                        ...
                        <ul class="list-tight">
                          @foreach(($result['determination']['subject_notifications'] ?? []) as $notice)
                            <li><strong>{{ $notice['audience'] ?? 'Data Subjects' }}</strong> — {{ $notice['deadline'] ?? '' }}<br><small class="text-muted">{{ $notice['message'] ?? '' }}</small></li>
                          @endforeach
                          @if(empty($result['determination']['subject_notifications']))
                  ...
                  <div class="summary-card mt-3">
                    ...
                    <ul class="list-tight mb-0">
                      @foreach($result['citations'] as $cite)
                        <li><strong>{{ $cite['regulation'] ?? '' }}</strong> — {{ implode(', ', $cite['clauses'] ?? []) }} <br><small class="text-muted">{{ $cite['reason'] ?? '' }}</small></li>
                      @endforeach
                    </ul>
                  </div>
                @endif

                @if(!empty($result['report_markdown']))
                  <div class="summary-card mt-3" id="reportMarkdown">
                    <h5>Full markdown report</h5>
                    <div class="bg-light rounded p-3" style="white-space:pre-wrap; font-family: 'IBM Plex Mono', monospace; font-size:.92rem;">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateFileMeta(){
    if(fileInput.files.length){
      ...
      fileMetaText.innerHTML = 'We’ll auto-convert PDFs/emails to text securely.';
    }
  }
  ...
  copyBtn && copyBtn.addEventListener('click', function(){
    const textSource = reportMarkdown ? reportMarkdown.innerText : document.body.innerText;
    navigator.clipboard.writeText(textSource).then(() => {
