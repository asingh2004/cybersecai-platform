						...
						<form method="post" action="{{url('listing/'.$result->id.'/'.$step)}}"  accept-charset='UTF-8' id="listing_bes">
							...
							<div class="form-row mt-4 border rounded pb-4">
								...
								<div class="form-group col-md-6 pl-5 pr-5">
  								...
  								<div class="form-group col-md-6 pl-5 pr-5">
									<label for="inputState">{{trans('messages.listing_basic.respite_care_type')}}
										<a href="https://myrespiteaccom.com.au/public/front/trainingguides/UnderstandingRespiteClassifications.pdf" target="_blank"><span class="fa fa-info-circle secondary-text-color"></span></a>
									</label>


									<select  name="respite_type"  class="form-control text-16 mt-2">
										@foreach($respite_type as $key => $value)
										<option value="{{ $key }}" {{ ($key == $result->respite_type) ? 'selected' : '' }}>{{ $value }}</option>
										@endforeach
									</select>
								</div>
