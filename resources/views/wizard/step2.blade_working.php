...
$(function() {
    ...
    @foreach($standards as $standard)
        ...
        idToFields[{{ $standard->id }}] = @json($standard->fields);
    @endforeach
    ...
    function renderFields() {
        ...
        selected.forEach(function(id){
            ...
            if (restoreMap.hasOwnProperty(standard)
                ...
                // Fallback: use 'fields' from standard def
                var fieldsRaw = idToFields[id] || '';
                fieldsArr = fieldsRaw.split(',').map(e => e.trim()).filter(Boolean);
            }

            var $tr = $('<tr></tr>');
            $tr.append('<td><strong>' + standard + '</strong></td>');
            $tr.append(
                '<td>' +
                (fieldsArr.length ? '<ul>' + fieldsArr.map(field => '<li>'+field+'</li>').join('') + '</ul>' : '<em>No fields defined.</em>') +
                '</td>');
            table.append($tr);
        });
    ...
    // On submit, populate regulations_json hidden
    $('#regulationForm').on('submit', function() {
        ...
        $('.regulation-checkbox:checked').each(function(){
            ...
            var standard = idToStandard[id];
            var fieldsRaw = idToFields[id] || '';
            var fieldsArr = fieldsRaw.split(',').map(e => e.trim()).filter(Boolean);
            regs.push({
                'standard': standard,
                'fields': fieldsArr
            });
