...
<div class="container">
  ...
  @php
    ...
    // Build the forgot password URL using $domain if provided, else fallback to settings/head_code or default.
    $base = rtrim($domain ?? ($settingsArr['head_code'] ?? 'https://mochanai.com/'), '/');
    $forgotUrl = $base . '/forgot_password';
  @endphp

  <p>
    Hi {{ $displayName }},<br><br>
    Welcome to <strong>{{ $settingsArr['name'] ?? 'our platform' }}</strong>! To set your password, click below:
  </p>

  <p style="margin:40px 0 0 0; text-align:center;">
    <a href="{{ $forgotUrl }}" class="btn" target="_blank">Set / Reset Password</a>
  </p>
