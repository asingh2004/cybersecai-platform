...
public function authenticate(Request $request)
{
    ...
    if ($user) {
        if ($user->status !== 'Active') {
            $this->helper->one_time_message('error', 'Your account is pending admin approval.');
            return redirect('login');
        }

        if ($user->email_verified_at === null) {
            $this->helper->one_time_message('error', 'Your email is not verified. Please check your inbox and verify your email.');
            return redirect('login');
        }

        if (Auth::validate(['email' => $request->email, 'password' => $request->password])) {
            $this->startTwoFactor($user, $remember, route('wizard.index'));
    ...
    public function verifyTwoFactor(Request $request)
    {
        ...
        // Force post-login destination to wizard.index
        $destination = route('wizard.index');
        ...
        if (Carbon::now()->greaterThan($token->expires_at)) {
            ...
            if ($user) {
                $this->startTwoFactor($user, $remember, $destination);
            }
            return redirect()->route('2fa.form');
        }
        ...
        // Always redirect to wizard.index after login
    ...
    protected function startTwoFactor(User $user, bool $remember = false, ?string $intended = null): void
    {
        // Force destination to wizard.index if not explicitly provided
        $intended = $intended ?? route('wizard.index');
        ...
        // Always use wizard.index as intended destination
        Session::put('2fa:intended', $intended);
    ...
    {
        if (!$request->isMethod('post')) {
            // Viewing the reset form

            $email = $request->query('email');

            $password_resets = PasswordResets::where('email', $email)
                                             ->where('token', $token);

            if ($password_resets->count()) {
                $password_result = $password_resets->first();

                // Check for token expiry as before
            ...
            if ($validator->fails()) {
                ...
                // Also delete if expired, could add logic if you want

                $password_resets = PasswordResets::where('token', $token_from_form)->delete();
    ...
    public function facebookAuthenticate(EmailController $email_controller, UserController $user_controller)
    {
        if (!isset(request()->error)) {
            ...
            if ($users->status != 'Inactive') {
                // Start 2FA for social login and send to wizard.index after verification
                $this->startTwoFactor($users, false, route('wizard.index'));
                return redirect()->route('2fa.form');
            } else {
                $data['title'] = 'Disabled ';
                return view('users.disabled', $data);
