...
class WebBlogSummarizationController extends Controller
{
    ...
    $summary = $this->summarizeTextWithPython($text, $request->detail);
    ...
    return back()
        ...
        ->with('summary', $displaySummary) // Pass the summarized text
        ->with('summary_pdf', $summaryPdfPath); // Pass the PDF filename
    }
    ...
    private function summarizeTextWithPython($text, $detail)
    {
        ...
        // Define the command to execute the Python script for summarization
        $command = "python3 /home/cybersecai/htdocs/www.cybersecai.io/public/python/summary/summarize.py " . escapeshellarg($textFilePath) . " " . escapeshellarg($detail) . " 2>&1"; // Capture STDERR
    ...
    private function writeSummaryToPdf($summary, $filename)
    {
        ...
        // Define the command to call the Python script for writing to PDF
        $command = "python3 /home/cybersecai/htdocs/www.cybersecai.io/public/python/summary/write_to_pdf.py " . escapeshellarg($summaryFilePath) . " " . escapeshellarg($filename) . " 2>&1"; // Capture STDERR
