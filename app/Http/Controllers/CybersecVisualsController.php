   ...
   protected function aggregateData($entity = null, $limit = 10000, $offset = 0)
{
    ...
    // For robust mapping: dropdown value => normalized _datasource
    $sourceMap = [
        'ALL' => 'ALL',
        'M365' => 'M365',
        'SMB'  => 'SMB',
        'S3'   => 'AWS S3', // IMPORTANT: dropdown S3 means _datasource 'AWS S3'
        'Microsoft 365' => 'M365',
        'AWS S3' => 'AWS S3',
        // add any more as needed
    ];
    $selectedSource = isset($entity, $sourceMap[$entity]) ? $sourceMap[$entity] : $entity;
    ...
    foreach ($configIDs as $id) {
        foreach (['M365', 'SMB', 'S3'] as $type) {
            $graphPath = "{$basePath}{$type}/{$id}/graph/";
            if (is_dir($graphPath))
                foreach (glob($graphPath . '*.json') as $jsonFile) $jsonFiles[] = $jsonFile;
        }
    }
    ...
    foreach ($jsonFiles as $file) {
        ...
        if (is_array($json) && isset($json[0]) && is_array($json[0])) {
            foreach ($json as &$record) {
                ...
                // Compute entity key as before, for dropdowns
                ...
                // --- MATCH ONLY THE SELECTED SOURCE (OR ALL)
                // $selectedSource example: 'AWS S3'
                if (
                    !$selectedSource ||
                    $selectedSource == 'ALL' ||
                    // Compare _datasource case-insensitively
                    (isset($record['_datasource']) && strcasecmp($record['_datasource'],$selectedSource)==0)
                ) {
            ...
            if (
                !$selectedSource ||
                $selectedSource == 'ALL' ||
                (isset($json['_datasource']) && strcasecmp($json['_datasource'],$selectedSource)==0)
            ) {
                if ($count++ < $offset) continue;
                if (count($data) >= $limit) break;
                $data[] = $json;
