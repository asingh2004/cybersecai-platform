...
class PropertyController extends Controller
{
    ...
    public function userFacilities(Request $request)
    {
        ...
        $data['daily_ndis_subscription_fee'] = DB::table('property_fees')
                                        ->where('field', '=', 'ndis_sta_daily_subs_fees')
                                        ->select('value')
                                         ->get();

        $data['supplier_type'] = DB::table('users')
                        ->where('users.id', Auth::id())
        ...
        if ($data['supplier_type'] === 'RACF') {
            session(['amount'  => $data['daily_subscription_fee']]);
        } else {
            session(['amount'  => $data['daily_ndis_subscription_fee']]);
        }
...
public function createBooking(Request $request)
    {
        ...
        $launch_date = Carbon::create(2022, 11, 1, 0);
...
public function stripeRequest(Request $request)
    ...
    public function create(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($validator->fails()) {
                ...
                $account_name = DB::table('users')
                                        ->where('users.id', Auth::id())
                                        ->pluck('first_name')->first();

                $property->name            = $account_name.' - '.' <<replace content in brackets with Service Type, ie High Care>>';
    ...
    public function listing(Request $request, CalendarController $calendar)
    {
        ...
        if ($step == 'basics') {
            if ($request->isMethod('post')) {
                ...
                $property->respite_service_type = RespiteType::where('id', $request->respite_type)->pluck('name')->first();
                ///$property->respite_type_fee = RespiteType::where('id', $request->respite_type)->pluck('fee')->first();
               ...
               /* if ($request->respite_type == 1){
                    ...
                    $property->respite_service_type = 'NDIS Respite';
                } elseif ($request->respite_type == 4){
                    $property->respite_service_type = 'Tour';
                } elseif ($request->respite_type == 5){
                    $property->respite_service_type = 'Vacancy Query';
                }else {
    ...
    public function single(Request $request)
    {
        ...
        $data['respite_service_name'] = Properties::where('respite_service_type','=', $request->respite_service_type)->first();
        //$data['respite_type_fee'] = RespiteType::where('name', '=', $data['respite_service_name'])->pluck('fee')->first();

        //dump($data['respite_service_name']);
        //dump($data['respite_type_fee']);
