...
class CustomerController extends Controller
{
   ...
   /* public function index(CustomerDataTable $dataTable)
    ...
    {
        // Only select your chosen columns for performance
        $data['customers'] = User::select([
  ...
  public function approveUser(Request $request, User $user)
{
    ...
    // Business logic for "admin": assign unique business_id (or keep if set through request)
    if ($roleName === 'admin') {
        if (empty($user->business_id)) {
            // Generate unique business_id (int)
            do {
        ...
        if ($request->filled('business_id')) {
            $user->business_id = (int)$request->input('business_id');
        }
        $user->save();
    }

    // If changing role to user, clear business_id if you wish, or keep for reference.
    if ($roleName !== 'admin') {
        // $user->business_id = null; // Uncomment if you want to clear when switching to user
        $user->save();
    }

    return back()->with('message', 'User approved and role assigned.');
}
...
public function editUserRole($userId)
{
    ...
    return view('admin.customers.edit_role', compact('user', 'roles'));
}
...
{
    $user  = User::findOrFail($userId);
    $role  = Roles::where('name', $request->input('role'))->firstOrFail();
    $user->roles()->sync([$role->id]);

    if ($role->name === 'admin') {
        // Allow superadmin to type or generate business_id
        $newId = $request->input('business_id') ?? '';
        if ($newId) {
            $user->business_id = (int)$newId;
        } else {
            // Generate unique on promotion
            do {
                $uniqueBusinessId = random_int(100000, 999999);
            } while (User::where('business_id', $uniqueBusinessId)->exists());
            $user->business_id = $uniqueBusinessId;
        ...
        // Optionally clear business_id or keep as is
        // $user->business_id = null;
        $user->save();
    }

    return redirect()->route('admin.users.editRole', $user->id)
        ->with('message', 'Role and business ID updated.');
}
    ...
    public function add(Request $request, EmailController $email_controller)
    {
        if (! $request->isMethod('post')) {
            ...
            if ($validator->fails()) {
                ...
                $user->point_of_contact = $request->point_of_contact;
    ...
    public function delete(Request $request)
    {
        ...
        if (env('APP_MODE', '') != 'test') {
            if ((count($properties)) && (count($bookings)) > 0) {
                $this->helper->one_time_message('danger', 'Customer have properties and bookings.Sorry can not  delete.');
            } else if (count($properties) > 0) {
                $this->helper->one_time_message('danger', 'Customer have properties.Sorry can not delete.');
            } else if (count($bookings) > 0) {
                $this->helper->one_time_message('danger', 'Customer have bookings.Sorry can not delete.');
            } else {
