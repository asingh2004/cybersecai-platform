...
class ScanM365Files extends Command
{
    protected $signature = 'scan:m365files {--max-workers=8 : Max workers for multiprocessing}';
    protected $description = 'Scan M365 OneDrive/SharePoint files via Python (multiprocessing-safe, resumable, writes compliance/secrets outputs)';

    public function handle()
    {
      	// The full scan runs only every two years
      	if (date('Y') % 2 !== 0) {
    		// Not an even year, don't scan.
    		return 0;
		}
        ...
        foreach ($configs as $config) {
            ...
            Log::info("[ScanM365] config_id=$config_id, regulations arg: $regulations");

            $subs_file = "/home/cybersecai/htdocs/www.cybersecai.io/webhook/subscriptions_bulk_{$config_id}.json";
            $script    = '/home/cybersecai/htdocs/www.cybersecai.io/webhook/M365/m365_scan.py';
            ...
            Log::info("[ScanM365] COMMAND for config $config_id: " . $cmd_shell);
            Log::info("[ScanM365] CMD ARRAY for config $config_id: " . json_encode($cmd));
            ...
            Log::info("[ScanM365] STDOUT (Config $config_id): " . $stdout);
            Log::info("[ScanM365] STDERR (Config $config_id): " . $stderr);
            Log::info("[ScanM365] Status (Config $config_id): $status");
        ...
        $summary = "M365 Files scan finished: $successes successes, $errors errors.";
        $this->info($summary);
        Log::info($summary);
