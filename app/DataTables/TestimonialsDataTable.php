        ...
        return datatables()
            ...
            ->addColumn('action', function ($testimonials) {
                ...
                if (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_testimonial')) {
                    $edit = '<a href="' . url('admin/edit-testimonials/' . $testimonials->id).'" class="btn btn-xs btn-primary"><span class="glyphicon glyphicon-edit"></span>Edit</a>&nbsp;';
                }
                if (Common::has_permission(\Auth::guard('admin')->user()->id, 'delete_testimonial')) {
                    $delete = '<a href="' . url('admin/delete-testimonials/' . $testimonials->id) . '" class="btn btn-xs     btn-danger delete-warning"><span class="glyphicon glyphicon-trash"></span>Delete</a>';
                }
                return $edit . $delete;
            })
