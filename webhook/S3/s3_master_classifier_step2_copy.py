...
region = s3config.get('AWS_REGION') or s3config.get('AWS_REGION', 'us-east-1')
s3 = boto3.client(
    "s3",
...
def fetch_file_content(bucket, record):
    ...
    try:
        ...
        # xls/xlsx
            ...
            tmpname = "/tmp/tmps3_%s.xlsx" % (os.urandom(4).hex())
            with open(tmpname, "wb") as tmpf:
                tmpf.write(response['Body'].read())
            try:
                df = pd.read_excel(tmpname, engine='openpyxl' if ext=='.xlsx' else None)
