...
//Fremium Demo
use App\Http\Controllers\AiComplianceBotController;
...
Route::get('/blog/{slug}', function($slug) {
    $valid = [
        'ai-gdpr-2025',
        'pitfalls-point-solutions',
        'audit-trail-smb',
      	'gdpr-agentic-automation'
    ];
    // Security: Only allow known slugs!
...
use App\Http\Controllers\CybersecVisualsController;

use App\Http\Controllers\WizardController;
use App\Http\Controllers\Compliance\EventController;
use App\Http\Controllers\CybersecPolicyController;
use App\Http\Controllers\CybersecSIEMController;
  
use App\Http\Controllers\AgenticAI\InternalAuditorController;
use App\Http\Controllers\AgenticAI\ComplianceAdvisorController;
use App\Http\Controllers\AgenticAI\PolicyEnforcerController;
  
use App\Http\Controllers\AgenticAI\DataBreachController;
use App\Http\Controllers\AgenticAI\DocsGeneratorController;

Route::middleware(['auth'])->group(function () {
    Route::get('/wizard/step1', [WizardController::class, 'step1'])->name('wizard.step1');
    Route::post('/wizard/step1', [WizardController::class, 'step1Post'])->name('wizard.step1.post');
    Route::get('/wizard/step2', [WizardController::class, 'step2'])->name('wizard.step2');
    Route::post('/wizard/step2', [WizardController::class, 'step2Post'])->name('wizard.step2.post');
    Route::get('/wizard/step3', [WizardController::class, 'step3'])->name('wizard.step3');
  	Route::post('/wizard/step3', [WizardController::class, 'step3Post'])->name('wizard.step3.post');
    Route::get('/wizard/step4', [WizardController::class, 'step4'])->name('wizard.step4');
    Route::post('/wizard/step4', [WizardController::class, 'step4Post'])->name('wizard.step4.post');
    Route::get('/wizard/step5', [WizardController::class, 'step5'])->name('wizard.step5');
    Route::post('/wizard/step5', [WizardController::class, 'step5Post'])->name('wizard.step5.post');
    Route::get('/wizard/done', [WizardController::class, 'done'])->name('wizard.done');
  	Route::get('/wizdashboard', [WizardController::class, 'dashboard'])->name('wizard.dashboard');
	Route::get('/config/{id}', [WizardController::class, 'show'])->name('wizard.show');
	Route::get('/config/{id}/edit', [WizardController::class, 'edit'])->name('wizard.edit');
	Route::delete('/config/{id}', [WizardController::class, 'destroy'])->name('wizard.destroy');
  
  
  	Route::get('/persona/auditor', [WizardController::class, 'auditorPersonaDashboard'])->name('persona.audit.dashboard');
  	Route::get('/persona/risk', [WizardController::class, 'riskPersonaDashboard'])->name('persona.risk.dashboard');
	Route::get('/persona/cybersecurity', [WizardController::class, 'cyberPersonaDashboard'])->name('persona.cyber.dashboard');
  

    Route::get('/persona/dashboard', [WizardController::class, 'personaComplianceDashboard'])->name('persona.dashboard');
    //Route::get('/persona/file/{fileId}', [WizardController::class, 'personaFileDetail'])->name('persona.file_detail');
  	//Route::get('/persona/file/{fileName}', [WizardController::class, 'personaFileDetail'])->name('persona.file_detail');

  	Route::get('/persona/file/{hash}/{fileName}', [WizardController::class, 'personaFileDetail'])
    ->name('persona.file_detail');
  
  	//Route::get('/dashboard/risk', [\App\Http\Controllers\WizardController::class, 'interactiveRiskDashboard'])->name('dashboard.risk');
  	//Route::post('/wizard/classify-files-m365/{config_id}', [WizardController::class, 'classifyFilesM365']);
  
  	// Discovery endpoints -- POST
	Route::post('/wizard/classify-files-m365/{config_id}', [WizardController::class, 'classifyFilesM365']);
	Route::post('/wizard/classify-files-smb/{config_id}', [WizardController::class, 'classifyFilesSMB']);
	Route::post('/wizard/classify-files-nfs/{config_id}', [WizardController::class, 'classifyFilesNFS']);
	Route::post('/wizard/classify-files-s3/{config_id}',  [WizardController::class, 'classifyFilesS3']);
	Route::post('/wizard/classify-files-gdrive/{config_id}', [WizardController::class, 'classifyFilesGDrive']);
  
  
  	// Classification endpoints -- POST
	Route::post('/wizard/start-classifying/{config_id}', [WizardController::class, 'startClassifying'])->name('wizard.start_classifying');
	Route::post('/wizard/start-classifying-smb/{config_id}', [WizardController::class, 'startClassifyingSMB']);
	Route::post('/wizard/start-classifying-nfs/{config_id}', [WizardController::class, 'startClassifyingNFS']);
	Route::post('/wizard/start-classifying-s3/{config_id}',  [WizardController::class, 'startClassifyingS3']);
	Route::post('/wizard/start-classifying-gdrive/{config_id}', [WizardController::class, 'startClassifyingGDrive']);
  
  
  	//Route::post('/wizard/start-classifying/{config_id}', [WizardController::class, 'startClassifying'])
    //->name('wizard.start_classifying');
  	Route::post('/wizard/start', [WizardController::class, 'startWizard'])->name('wizard.start');
  
  	Route::get('/wizard/filesummary_pyramid', [App\Http\Controllers\WizardController::class, 'fileSummaryPyramid'])
    ->name('wizard.filesummary_pyramid');
  	
  	Route::get('/wizard/visuals-dashboard', function() {
    	return view('wizard.visuals_dashboard');
		})->name('wizard.visuals_dashboard');
  	Route::get('/wizard/file-graph/network', [\App\Http\Controllers\WizardController::class, 'fileGraphNetwork'])->name('wizard.file_graph_network');
	Route::get('/wizard/file-graph/table', [\App\Http\Controllers\WizardController::class, 'fileGraphTable'])->name('wizard.file_graph_table');

   	Route::post('/wizard/establish-m365-link/{config_id}', [WizardController::class, 'establishM365Link'])->name('wizard.establish_m365_link');
    ...
    Route::get('/cybersecai-visuals/api/filedata', [CybersecVisualsController::class, 'apiFileData']);
  	...
  	/*Route::prefix('gdpr/events')->group(function() {
        Route::get('/', [EventController::class, 'index'])->name('gdpr.events.index');
        Route::get('/create', [EventController::class, 'create'])->name('gdpr.events.create');
        Route::post('/store', [EventController::class, 'store'])->name('gdpr.events.store');
        Route::get('/{id}', [EventController::class, 'show'])->name('gdpr.events.show');
    });*/
	...
	// 1. SHOW generation form (Blade 1, GET)
  	// 2. GENERATE (POST from Blade 1)
  	// 3. VIEW grouped/downloadable/generated docs (Blade 2, GET)
  	// 4. DOWNLOAD endpoints
  	Route::get('/agenticai/docs_agent/form', [\App\Http\Controllers\AgenticAI\DocsGeneratorController::class, 'showForm'])->name('agenticai.docs_agent.form');
	Route::post('/agenticai/docs_agent/generate', [\App\Http\Controllers\AgenticAI\DocsGeneratorController::class, 'generate'])->name('agenticai.docs_agent.generate');
	Route::get('/agenticai/docs_agent', [\App\Http\Controllers\AgenticAI\DocsGeneratorController::class, 'index'])->name('agenticai.docs_agent.index');
	Route::get('/agenticai/docs/json_download/{user_id}/{filename}', [\App\Http\Controllers\AgenticAI\DocsGeneratorController::class, 'jsonDownload'])->where('filename', '.*')->name('agenticai.docs.json_download');
	Route::get('/agenticai/docs/docx_download/{user_id}/{filename}', [\App\Http\Controllers\AgenticAI\DocsGeneratorController::class, 'docxDownload'])->where('filename', '.*')->name('agenticai.docs.docx_download');
	Route::post('/agenticai/docs_agent/delete', [\App\Http\Controllers\AgenticAI\DocsGeneratorController::class, 'deleteDocument'])->name('agenticai.docs_agent.delete');
  
  	Route::get('/agentic_ai/agents', [\App\Http\Controllers\AgenticAI\AgentListController::class, 'index'])
    ->middleware(['auth']) // if using authentication
    ->name('agentic_ai.agents');
  
  	
/*	Route::get('/agentic_ai/databreach', [DataBreachController::class, 'showForm'])->name('agentic_ai.databreach.form');
	Route::post('/agentic_ai/databreach', [DataBreachController::class, 'generateBreachDocuments'])->name('agentic_ai.databreach.run');

  	
  
  	// Show the generator page
  	Route::get('/agenticai/docs-agent', [DocsGeneratorController::class, 'index'])->name('agenticai.docs_agent.index');
  // routes/web.php (add to your routes)
  	// routes/web.php
	Route::get('/agenticai/docs/jsondownload/{user_id}/{filename}', [\App\Http\Controllers\AgenticAI\DocsGeneratorController::class, 'jsonDownload'])->where('filename', '.*')->name('agenticai.docs.json_download');
	Route::get('/agenticai/docs/docxdownload/{user_id}/{filename}', [\App\Http\Controllers\AgenticAI\DocsGeneratorController::class, 'docxDownload'])->where('filename', '.*')->name('agenticai.docs.docx_download');
	
  	// Run the AI doc generation and show results (POST)
	Route::post('/agenticai/docs-agent/generate', [DocsGeneratorController::class, 'generate'])->name('agenticai.docs_agent.generate');
  	// Download a generated file by session/filename
	Route::get('/agenticai/docs-agent/download/{session}/{filename}', [DocsGeneratorController::class, 'download'])->name('agenticai.docs.download');
*/  
});



	use App\Http\Controllers\CybereSecAiAgentsController;
...
// Define the route for the Playground page
Route::get('/playground', [PlaygroundController::class, 'index']);

use App\Http\Controllers\MathTutorController;
Route::get('/math-tutor', [MathTutorController::class, 'index']);
Route::post('/math-tutor', [MathTutorController::class, 'getAnswer'])->name('math-tutor.answer');


use App\Http\Controllers\PdfSummarizationController;
//Route::get('/pdf-summarization', [PdfSummarizationController::class, 'index'])->name('pdf.index');
Route::get('/pdf/index/{id}', [PdfSummarizationController::class, 'index'])->name('pdf.index');
Route::post('/pdf/summarize/{id}', [PdfSummarizationController::class, 'summarize'])->name('pdf.summarize');


use App\Http\Controllers\WebBlogSummarizationController;
Route::get('/blog-summarizer', [WebBlogSummarizationController::class, 'index'])->name('web.blog.summarizer');
Route::post('/blog-summarize', [WebBlogSummarizationController::class, 'summarize'])->name('web.blog.summarize');


use App\Http\Controllers\OpenAIController;
// Route to show the OpenAI configuration form
Route::get('/openai/create', [OpenAIController::class, 'create'])->name('openai.create');

// Route to handle the OpenAI configuration form submission
Route::post('/openai/configure', [OpenAIController::class, 'store'])->name('openai.configure');

Route::get('/assistant/{id}', [OpenAIController::class, 'showAssistantTemplate'])->name('assistant.template');
Route::post('/assistant/submit', [OpenAIController::class, 'submitMessage'])->name('assistant.submit');

// Routes for assistants
Route::get('user/assistants', [OpenAIController::class, 'userAssistants'])->name('user.assistants');
Route::get('/assistants/{id}/instructions', [OpenAIController::class, 'getInstructions'])->name('assistants.instructions');
Route::get('assistants/{id}/basics', [OpenAIController::class, 'show'])->name('assistants.show');
Route::put('openai/configure/{id}', [OpenAIController::class, 'update'])->name('openai.update');
Route::post('assistants/update_status', [OpenAIController::class, 'updateStatus'])->name('assistants.updateStatus');

// Route for showing the assistant details and rendering the completion template
Route::get('assistants/{id}/completion-template', [OpenAIController::class, 'showCompletionTemplate'])->name('assistant.completionTemplate');

// Route for handling the completion form submission
Route::post('assistants/completion-template', [OpenAIController::class, 'handleCompletion'])->name('assistant.handleCompletion');

use App\Http\Controllers\OpenAIAssistantController;
// Route to access the AI Assistant page
Route::get('/openai/assistant', function () {
    return view('openai.ai-assistant');
})->name('openai.assistant');

// Route to create an assistant
Route::post('/assistant/create', [OpenAIAssistantController::class, 'createAssistant'])->name('assistant.create');

// Route to upload a file
Route::post('/files/upload', [OpenAIAssistantController::class, 'uploadFile'])->name('files.upload');

// Route to list files
Route::get('/files', [OpenAIAssistantController::class, 'listFiles'])->name('files.list');

// Route to delete a file
Route::delete('/files/delete', [OpenAIAssistantController::class, 'deleteFile'])->name('files.delete');

// Route to create a thread
Route::post('/threads/create', [OpenAIAssistantController::class, 'createThread'])->name('threads.create');
